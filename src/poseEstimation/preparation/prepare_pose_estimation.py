import sys
import os
sys.path.append(os.path.abspath("../../"))   #注意修改
...
from poseEstimation.datasets import DatasetDetections
from poseEstimation.datasets.augmentation import CropToBox
from poseEstimation.pose_estimator.pose_estimator_hrnet import PoseEstimatorHRNet
from poseEstimation.pose_estimator.utils import *
from poseEstimation.visualization.utils import keypoints
...
def pose_estimation(dataset_base_path, detection_list, output_file):
    ...
    #num_workers = 8改为 0
    data_loader = DataLoader(dataset, batch_size=200, shuffle=False, num_workers=0)
    print(f"Data loaded: {len(data_loader)} batches")

    file = open(output_file, "w",newline="")  #避免出现多空行
